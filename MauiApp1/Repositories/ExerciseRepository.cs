...
namespace SpinningTrainer.Repositories
{
    public class ExerciseRepository : RepositoryBase, IExerciseRepository
    {
        ...
        public ExerciseModel Add(ExerciseModel exercise)
        {
            ...
            {
                string query = @"INSERT INTO Movimiento (Descrip, ZonasDeEnergia, RPMMin, RPMMax, PosicionesDeManos)
                                 VALUES (@Descrip, @ZonasDeEnergia, @RPMMin, @RPMMax, @PosicionesDeManos);
                                 SELECT SCOPE_IDENTITY();";

                SqlCommand command = new SqlCommand(query, connection);
                command.Parameters.AddWithValue("@Descrip", exercise.Descrip);
                command.Parameters.AddWithValue("@ZonasDeEnergia", exercise.ZonasDeEnergia);
            ...
            using (SqlConnection connection = OpenConnection())
            {
                ...
                if (reader.Read())
                {
                    exercise = new ExerciseModel
                    {
                        ...
                        ZonasDeEnergia = Convert.ToString(reader["ZonasDeEnergia"]),
